...
Tools installed (pinned versions, see versions.json):
   ...
   * Python           3.14.3
   * AWS CLI v2       latest
   * Claude Code      stable    (CLI, via official installer)
...
--------------------------
   ...
   Press Enter. The script will:
      - FIRST install Claude Code via its official installer
        (irm https://claude.ai/install.ps1 | iex) into
        C:\Users\<you>\.local\bin  (so the CLI is ready even
...
Install ONLY Claude Code (skip Git/Node/Python/AWS):
...
Install everything EXCEPT Claude Code:
   ...
   powershell -ExecutionPolicy Bypass -File .\install.ps1 -SkipClaude
...
entries (including the AWS CLI PATH entry). Nothing system-wide
is touched.

Intentionally LEFT in place (each has its own removal path):
   - Claude Code (.local\bin) - run "claude uninstall" to remove.
   - The AWS CLI v2 app itself - remove via Settings > Apps.
   - .npmrc - edit/remove by hand if you no longer want it.
...
"claude: command not found"
   Run:  echo $env:Path
   Confirm that the path ending in ".local\bin" is present
   (that is where the official installer puts claude.exe). If
   not — re-open PowerShell, or run activate.ps1. You can also
   re-run the installer directly:
...
================================================================
   ...
   C:\Users\<you>\ai-devkit\
      ...
      (note: AWS CLI does NOT live here - see below)

   C:\Users\<you>\AppData\Local\Programs\Amazon\AWSCLIV2\
      aws.exe - the AWS CLI v2 per-user MSI ignores any custom
      install dir and always lands here. (If the MSI is blocked,
      the fallback "pip install awscli" goes to ai-devkit\python\
      Scripts instead.)

   C:\Users\<you>\.local\bin\
      claude.exe - installed by Claude Code's own official
      installer (not under ai-devkit\). The installer also adds
      this dir to your user PATH.
   ...
   User PATH (HKCU\Environment\Path)
      Seven entries: git\cmd, node, npm-global, python,
      python\Scripts, the AWS CLI dir (AppData\Local\Programs\
      Amazon\AWSCLIV2), and .local\bin. uninstall.ps1 removes the
      ai-devkit entries AND the AWS entry, but leaves .local\bin
      in place because Claude Code's own installer manages it.
